% METRIC_MSE computes the mean squared error results
%
% Copyright (c) 2026 Robert Bosch GmbH
% SPDX-License-Identifier: AGPL-3.0

function result = metric_mse(x,u,qoi_name,plant_name,quantile_val)
% returns mean, variance and quantiles of the mean squared error of x or u
...
%   quantile_val    : lower quantile to be computed
    ...
    for i=1:size(qoi,3)
        ...
        result_vec(i,:) = 1/(num_steps-(init_complete_idx-1)) .* sum((qoi(init_complete_idx:end,:,i)).^2,1);
    end
    for i=1:size(qoi,2)
