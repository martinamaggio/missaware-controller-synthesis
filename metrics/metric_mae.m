% METRIC_MAE computes the mean absolute error results
%
% Copyright (c) 2026 Robert Bosch GmbH
% SPDX-License-Identifier: AGPL-3.0

function result = metric_mae(x,u,qoi_name,plant_name,quantile_val)
% returns mean, variance and quantiles of the mean absolute error of x or u
...
%   quantile_val    : lower quantile to be computed
    ...
    for i=1:size(qoi,3)
        ...
        result_vec(i,:) = 1/(num_steps-(init_complete_idx-1)) .* sum(abs(qoi(init_complete_idx:end,:,i)),1);
    end
    for i=1:size(qoi,2)
